...
class ViewController: UIViewController {
    //MARK: VAR && LET
    // temp
    let nameFieldArray = ["foreign","original","level","date","image","mistake","simple"]
    let nameTable = "users"
    // temp end
    // DataBase
    ...
    var detailPage = UIView() // edit word's card
...
//                                        .writeFromTranslate,
    ...
    // original cardForLevel3
//    let cardForLevel3 : [TypeOfCard] = [.prononceChooseTranslate,
//                                        .writeFromPrononce,
//                                        .writeFromTranslate]
    // Block bad prononce BEGIN
//    let cardForMeeting : [TypeOfCard] = [.meeting,
//                                         .trueOrFalse,
//                                         .pictureAndTranslateChooseWrite,
//                                         .pictureChooseWrite]
...
//                                        .prononceAndWriteChooseTranslate,
//                                        .writeChoosePicture]
//    let cardForLevel2 : [TypeOfCard] = [.prononceAndWriteChooseTranslate,
//                                        .writeFromTranslate,
//                                        .writeChoosePicture]
//
//    let cardForLevel3 : [TypeOfCard] = [.writeFromTranslate,.writeFromTranslate]
    // Block bad prononce END
    
    var numberCard = 0
    var correctAnswer = ""
    var countWordInOneIteration = 15
...
//        print("End")
    ...
    // MARK: - Modify DB
    func singIn() {
        Auth.auth().signIn(withEmail: "anton.rubenchik@gmail.com", password: "123456") { (user, error) in
            if let error = error {
    ...
    func writeToFireBase() {
        ...
        if userID != nil {
            ...
            for item in wordsCoreDataArray {
                // create dictionary with information about word
                let dateDiff = Calendar.current.dateComponents([.day], from: dateFrom, to: item.nextContact! as Date).day
                
                var wordDict = [String : Any]()
                for i in 0...nameFieldArray.count-1 {
                    switch i {
                    case 0: wordDict[nameFieldArray[i]] = item.foreign
                    case 1: wordDict[nameFieldArray[i]] = item.original
                    case 2: wordDict[nameFieldArray[i]] = item.levelOfLearning
                    case 3: wordDict[nameFieldArray[i]] = dateDiff
                    case 4: wordDict[nameFieldArray[i]] = item.image
                    case 5: wordDict[nameFieldArray[i]] = item.mistake
                    case 6: wordDict[nameFieldArray[i]] = item.simple
                    default:
                        return
                    }
                }

                self.ref.child(nameTable).child(userID!).child(item.image!).setValue(wordDict) { (error, dataResult) in
                    if let error = error {
...
//                    print("\(word.foreign!)\n\(word.original!)\n\(word.levelOfLearning)\n\(dateDiff!)\n\(word.image!)\n\(word.mistake)\n\(prononce)\n")
    ...
    private func createLabel(posX: Int, posY: Int, text: String, size: Int, color: UIColor)-> UILabel {
        ...
        var posXnew = posX
        if posXnew == 0 {
            posXnew = Int(self.view.center.x - newLabel.frame.width/2)
        }
        newLabel.frame = CGRect(x: CGFloat(posXnew), y: CGFloat(posY), width: newLabel.frame.width, height: newLabel.frame.height)
...
//            }
        ...
        case .edit:
            ...
            newButton.frame = CGRect(x: CGFloat(posX - posY/10), y: CGFloat(posY - posY/5), width: CGFloat(posY/20), height: CGFloat(posY/20))
//            newButton.tintColor = .white
            if let newImage = UIImage(named: "design.png") {
                newButton.setImage(newImage, for: .normal)
            }
//            newButton.backgroundColor = backgroundForWord
//            newButton.addTarget(self, action: #selector(pronunciationWord), for: .touchDown)
    ...
    func createTextField(posY: Int, text: String) -> UITextField {
        let newTextField = UITextField()
        newTextField.frame = CGRect(x: 20, y: posY, width: Int(self.view.bounds.width) - 40, height: 34)
        newTextField.text = text
        newTextField.borderStyle = .bezel
...
//        let speakMsg    = AVSpeechUtterance(string: ("computer"))
//        let speakMsg    = AVSpeechUtterance(string: ("מַלוֹן"))
        speakMsg.voice  = AVSpeechSynthesisVoice(language: "he-IL")
//        speakMsg.voice  = AVSpeechSynthesisVoice(language: "en-US")
    ...
    @objc func pressedSave(){
        ...
        // look for word for editing
        let card = wordForLearningInThisIteration[numberCard]
        let editWord = wordArray[card.0]
        
        // Begin. Fill array from textField and Switch
        var textFieldInformation : [String] = []
        var boolSwitchInformation : [Bool] = []
        
        let subViews = detailPage.subviews
        for item in subViews {
            if let textField = item as? UITextField {
                textFieldInformation.append(textField.text ?? "error")
            }
            
            if let switchX = item as? UISwitch {
                boolSwitchInformation.append(switchX.isOn)
            }
        }
        // End. Fill array from textField and Switch

        if (textFieldInformation.count == 3) && (boolSwitchInformation.count == 2) {
            // save new data
            // upgrade in array with word in this iteration
            for i in 0...(wordArray.count - 1) {
                if wordArray[i].image == editWord.image {
                    wordArray[i].foreign =  textFieldInformation[0]
                    wordArray[i].original = textFieldInformation[1]
                    wordArray[i].simple = textFieldInformation[2]
                    print("Correct information in CurrentArray")
                    if boolSwitchInformation[0] == true {
                        wordArray[i].levelOfLearning = 0
                    }
                    if boolSwitchInformation[1] == true {
                        wordArray[i].dontWantToLearn = true
                    }
                }
            }
...
//            if convertHebrewString(word: chooseWord.lowercased()) == convertHebrewString(word: correctAnswer.lowercased()) {
    ...
    private func createMeetingCard(word: WordForLearning) {
        ...
        let foreignLabel = createLabel(posX: 20, posY: 7, text: word.foreign, size: 35, color: .white)
        ...
        let editButton = createButton(type: TypeOfButton.edit,
        ...
        editButton.addTarget(self, action: #selector(pressedEdit), for: .touchDown)
        ...
        prononceLabel = createLabel(posX: 20, posY: Int(meetCardView.frame.height/100*63), text: "test", size: 25, color: .black)
        // create meetCard
        meetCardView.addSubview(nextButton)
        meetCardView.addSubview(editButton)
    ...
    private func createType2Card(word: WordForLearning) {
        ...
        let rememberLabel = createLabel(posX: Int(type2CardView.frame.width/10*6),
                                      posY: Int(type2CardView.frame.height/28*25),
                                      text: "Remember",
                                      size: 18, color: backgroundDoneButton)
    ...
    private func createType3Card(word: WordForLearning) {
        ...
        let foreignLabel = createLabel(posX: 20, posY: 10, text: "Remember the word", size: 35, color: colorHolderForWord)
        let russianLabel = createLabel(posX: 20,
                                       posY: Int(fieldForWord.frame.height/2+5),
                                       text: "",//word.original,
            size: 25, color: .white)
    ...
    private func createType4Card(word: WordForLearning) {
        ...
        let foreignLabel = createLabel(posX: 20, posY: 10, text: word.foreign, size: 35, color: .white)
        let originalLabel = createLabel(posX: 20,
                                       posY: Int(fieldForWord.frame.height/2+5),
                                       text: "Remember the translation",
                                       size: 25, color: colorHolderForWord)
    ...
    private func createStartPage() {
        ...
        let informationLabel = createLabel(posX: 20, posY: Int(startPage.frame.height/3), text: "Information", size: 22, color: .black)
        let progressInformation = createLabel(posX: 20, posY: 40, text: "BlackAndLeft", size: 18, color: .black)
    ...
    private func createDetailPage() {
        
        // create Card View
        detailPage.isHidden = true
        detailPage.bounds.size = view.bounds.size
        detailPage.center = view.center
        let titelLabel = createLabel(posX: 0, posY: 40, text: "Edit", size: 28, color: .black)
        let wordLabel = createLabel(posX: 20, posY: Int(detailPage.frame.height*0.15), text: "Word", size: 22, color: .black)
        let wordTextField = createTextField(posY: Int(detailPage.frame.height*0.19), text: "Table")
        
        let translateLabel = createLabel(posX: 20, posY: Int(detailPage.frame.height*0.3), text: "Translate", size: 22, color: .black)
        let translateTextField = createTextField(posY: Int(detailPage.frame.height*0.34), text: "Translate")
        
        let prononceLabel = createLabel(posX: 20, posY: Int(detailPage.frame.height*0.45), text: "Prononce", size: 22, color: .black)
        let prononceTextField = createTextField(posY: Int(detailPage.frame.height*0.49), text: "Prononce")
        
        let resetLevelLabel = createLabel(posX: 20, posY: Int(detailPage.frame.height*0.60), text: "Reset level", size: 22, color: .black)
        let resetSwitch = UISwitch(frame: CGRect(x: Int(detailPage.frame.width-75), y: Int(detailPage.frame.height*0.60), width: 0, height: 44))
        let dontLearnLabel = createLabel(posX: 20, posY: Int(detailPage.frame.height*0.70), text: "Don't learn", size: 22, color: .black)
        let dontLearnSwitch = UISwitch(frame: CGRect(x: Int(detailPage.frame.width-75), y: Int(detailPage.frame.height*0.70), width: 0, height: 44))
        
        let saveButton = createButton(type: TypeOfButton.next,
                                      posX: 0,
                                      posY: Int(detailPage.frame.height/10*9),
                                      text: "Сохранить",
                                      size: 20,
                                      color: UIColor(red: 153/255, green: 204/255, blue: 102/255, alpha: 1))
        saveButton.addTarget(self, action: #selector(pressedSave), for: .touchDown)
        
        detailPage.addSubview(saveButton)
        detailPage.addSubview(titelLabel)
        detailPage.addSubview(wordLabel)
        detailPage.addSubview(wordTextField)
        detailPage.addSubview(translateLabel)
        detailPage.addSubview(translateTextField)
        detailPage.addSubview(prononceLabel)
        detailPage.addSubview(prononceTextField)
        detailPage.addSubview(resetLevelLabel)
        detailPage.addSubview(dontLearnLabel)
        detailPage.addSubview(resetSwitch)
        detailPage.addSubview(dontLearnSwitch)
        
        self.view.addSubview(detailPage)
    }
    
    // create prototype of card
...
//    Написание! картинка?   (выбор из 4-х картинок)                     type 7
    ...
    func showDetailPage(word: WordForLearning) {
        // Страница редактирования

        let subViews = detailPage.subviews
        var numberTextField = 0
        for item in subViews {
            if let textField = item as? UITextField {
                switch numberTextField {
                case 0: textField.text = word.foreign
                case 1: textField.text = word.original
                case 2: textField.text = word.simple
                default: textField.text = "Star"
                }
                numberTextField += 1
            }
    ...
    func addNewWords() {
        ...
        for wordFromAddArray in wordsFromAddArray.newWordsArray {
            ...
            // The block corrects new words, after Ivan learning. My words changed after it.
//            for temp in filteredArray {
//                temp.nextContact = Date() as NSDate
//                temp.levelOfLearning = 0
//                temp.mistake = 0
//            }
//            CoreDataManager.instance.saveContext()
...
//                }
    ...
    private func createWordsArray() {
        ...
        // Определяем сколько новых слов в базе (до 3-х] встреч)
        var newWordsArray = wordsCoreDataArray.filter({ (word) -> Bool in
            return word.levelOfLearning > 2 ? false : true // !=0
        })
        
        var oldWordsArray = wordsCoreDataArray.filter({ (word) -> Bool in
            return word.levelOfLearning > 2 ? true : false // !=0
        })
        ...
        // The Block distrubutes how many old and new words in this iteration
        // 0.4 and 0.6
        if newWordsArray.count > Int(Double(countWordInOneIteration) * 0.8) {
            if (oldWordsArray.count) > Int(Double(countWordInOneIteration) * 0.2) {
                new = Int(Double(countWordInOneIteration) * 0.8)
                old = Int(Double(countWordInOneIteration) * 0.2)
            } else {
...
//        staticInformation.countWord =
        ...
        for wordFromArray in wordArray {
            ...
            let request = NSFetchRequest<NSFetchRequestResult>(entityName: "Words")
            let predicateFirst = NSPredicate(format: "%K == %@", "image", wordFromArray.image)
